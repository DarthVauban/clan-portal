...
import { listPortalCollectiveState, savePortalCollectiveState } from "@/lib/portal-collective-repository";
...
export async function PUT(request: NextRequest) {
  ...
  const state = await savePortalCollectiveState(session, payload?.state);
  if (!state) {
    return NextResponse.json({ state: null }, {
      status: 403,
      headers: { "Cache-Control": "no-store" },
