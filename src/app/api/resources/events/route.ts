...
import { hasPortalResourceAccess } from "@/lib/portal-resource-repository";
import { subscribePortalResourceEvents } from "@/lib/portal-resource-events";
...
export async function GET(request: NextRequest) {
  ...
  if (!(await hasPortalResourceAccess(session))) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      unsubscribe = subscribePortalResourceEvents(() => send("resources-changed", { at: Date.now() }));
