...
export function ResourceRequestsManager({ resources }: { resources: ResourceCatalogItem[] }) {
  ...
  const visibleResources = resources.filter((resource) => !normalizedQuery
    || [resource.name, resource.englishName].some((name) => name.toLocaleLowerCase("ru").includes(normalizedQuery))).slice(0, 80);
